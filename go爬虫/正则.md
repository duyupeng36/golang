...
```
`FindString`返回保管正则表达式`re`在`s`中的最左侧的一个匹配结果的字符串。
...
```
`FindAllString`返回保管正则表达式`re`在`s`中的所有不重叠的匹配结果的`[]string`切片。
如果没有匹配到，会返回`nil`。
* `n`表示匹配次数，`-1`表示匹配所有
...
```
`FindStringIndex`返回保管正则表达式`re`在`s`中的最左侧的一个匹配结果的**起止位置**的切片
...
```
`ReplaceAllString`返回`src`的一个拷贝，**将`src`中所有`re`的匹配结果都替换为
...
```
`ReplaceAllStringFunc`返回`src`的一个拷贝，将`src`中所有`re`的匹配结果
